...
export default async () => {
  ...
  const caller = '[ServiceCall]-revokeAllNamadCertificates';

  try {
    onApiCallingStart(caller);
    const response: ApiResponseModel<CertificateServerResponseModel> =
      await fetchJSON();
...
const fetchJSON = () => {
  ...
  return ApiCaller(options).delete(
    `${SERVER_URL}api/certificate/namad/revoke/all`,
  );
};
