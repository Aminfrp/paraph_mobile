...
import {certificateFileData, compareCertificateWithFile} from "../../../../modules/certificate/rishe/certificate";
...
const Index: React.FC<PropsModel> = props => {
  ...
  useEffect(()=> {
      certificateFileData().then(r => r ? setIsCertificateExist(false):setIsCertificateExist(true) );
  },[])
  ...
  return (
    <View style={styles.certificateDataCard}>
      ...
      {isCertificateExist || showCertificate ?<View style={styles.certificateDataCardDivider}/>:null}

        {isCertificateExist && <View style={styles.deviceAndRisheCertificateExistContainer}>
            <Text style={styles.deviceCertificateExistText}>این گواهی ریشه هم‌اکنون روی دستگاه دیگری فعال است و در این
                دستگاه معتبر نیست. برای استفاده از گواهی در این دستگاه، لطفاً ابتدا گواهی قبلی را ابطال کنید و سپس برای
                خرید گواهی جدید اقدام نمایید. در غیر این صورت، از گواهی موجود روی دستگاه قبلی استفاده کنید.
            </Text>
            <Image
