...
type PropsModel = {
  ...
  isRisheCertificateExist: boolean;
  isNamadCertificateExist: boolean;
...
const Index: React.FC<PropsModel> = props => {
  const {
    onInfo,
    isRisheCertificateExist,
    isNamadCertificateExist,
    onRevoke,
    certificates,
  } = props;
  ...
  // @ts-ignore
  const onRisheHistory = () => navigate(RISHE_CERTIFICATE_PAYMENT_HISTORY);
  ...
  const getCertDate = (item: CertificateModel) => {
    ...
    return `معتبر تا تاریخ ${new Date(
